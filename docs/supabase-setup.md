> Обновление 04.09: вход переведён на email + пароль (magic link на бесплатном тарифе упирается в 2 письма/час и требует того же браузера). В Auth включён `mailer_autoconfirm`, письма при регистрации не отправляются. Всё ниже про redirect URL уже настроено через Management API.
